...
public class SellDAO {
	...
	public void buy(String sellId, MemberDTO member) {
		...
		try {
			...
			// SQL文
			String sql = "UPDATE sell SET sell_status = '振込まち',purchaser_member_id = " + memberId + " WHERE sell_id = "
					+ sellId;

			System.out.println(sql);
			// UPDATE `sell` SET `purchaser_member_id` = '2', `sell_status` = '振込まち' WHERE
			// `sell`.`sell_id` = 2;
			// SQLをDBへ発行
	...
	// DBから商品画像と商品名と値段を検索
	public ArrayList<SellDTO> selectByPurchase(int purchase) {
		// return用オブジェクト生成
		ArrayList<SellDTO> list = new ArrayList<SellDTO>();
		// SQL文
		String sql = "SELECT * FROM sell WHERE purchaser_member_id = " + purchase;
		System.out.println(sql);
	...
	public int itemInsert(SellDTO sell) {
		...
		try {
			// insert文の作成
			String sql = "INSERT INTO sell(seller_member_id, purchaser_member_id, product_name, product_type, price, prefectures, remarks, delivery_days, product_condition, image_path, sell_status, sell_date, purchase_date, transfer_date, shipment_date, completion_date) VALUES"
					+ "(?, NULL, ?, ?, ?, ?, ?, ?, '未使用', ?, ?, ?,NULL,NULL,NULL,NULL );";
			con = MemberDAO.getConnection();
			psmt = con.prepareStatement(sql);
			// ?の値を決める
			psmt.setInt(1, sell.getSellerMemberId());
			;
			psmt.setString(2, sell.getProductName());
			psmt.setString(3, sell.getProductType());
			psmt.setInt(4, sell.getPrice());
			psmt.setString(5, sell.getPrefectures());
			psmt.setString(6, sell.getRemarks());
			psmt.setInt(7, sell.getDeliveryDays());
			psmt.setString(8, sell.getImagePath());
			psmt.setString(9, "出品中");
	...
	public void insert(int memberId, String sellId) {
		...
		try {
			...
			//SQL文
			String sql = "INSERT INTO favolite (sell_id,member_id) VALUES('"
					+ sellId +"','"
					+ memberId +"')";
